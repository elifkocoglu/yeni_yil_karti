...
const ClipStudio: React.FC = () => {
    ...
    const startPreview = () => {
        ...
        // (In a real app, we would interleave stock images here, but keeping it simple for stability)
                ...
                <div className="lg:col-span-5 space-y-6">
                    ...
                    {photos.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {photos.map((p, i) => (
                                <img key={i} src={p} className="w-16 h-16 object-cover rounded-lg border border-white/20 flex-shrink-0" />
                            ))}
                        </div>
                    )}
                </div>
                ...
                <div className="lg:col-span-7 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {status === 'preview' ? (
                            <button
                                onClick={stopPreview}
                                className="py-4 rounded-xl bg-gray-700 text-white font-bold hover:bg-gray-600 transition-colors"
                            >
                                Kareyi Dondur ⏸️
                            </button>
                        ) : (
                            <button
                                ...
                                className="py-4 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                                Önizle 👁️
                            </button>
                        )}
